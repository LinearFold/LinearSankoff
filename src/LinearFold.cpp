...
#ifdef lv
    bool comparefunc(pair<int,LFState> a, pair<int,LFState> b) {
        return a.first > b.first;
    }

    void BeamCKYParser::sort_keys(unordered_map<int, LFState> &map, vector<pair<int,LFState>> &sorted_keys) {
        sorted_keys.clear();
        for(auto &kv : map) {
...
void BeamCKYParser::get_parentheses(char* result, const string& seq) {
    ...
    stack<tuple<int, int, LFState>> stk;
    ...
    while ( !stk.empty() ) {
        tuple<int, int, LFState> top = stk.top();
        int i = get<0>(top), j = get<1>(top);
        LFState& state = get<2>(top);
        stk.pop();
        ...
        switch (state.manner) {
            ...
            case LFMANNER_MULTI: 
                ...
                break;
            case LFMANNER_MULTI_eq_MULTI_plus_U:
                p = i + state.trace.paddings.l1;
                q = j - state.trace.paddings.l2;
                stk.push(make_tuple(p, q, bestM2[q][p]));
                break;
            case LFMANNER_P_eq_MULTI:
                result[i] = '(';
                result[j] = ')';
...
value_type BeamCKYParser::beam_prune(unordered_map<int, LFState> &beamstep) {
    scores.clear();
    for (auto &item : beamstep) {
        int i = item.first;
        LFState &cand = item.second;

        // if (cand.manner == LFMANNER_HAIRPIN) continue;
...
void BeamCKYParser::sortM(value_type threshold,
    ...
    sort(sorted_stepM.begin(), sorted_stepM.end(), greater<pair<value_type, int>>());
}

//subopt zuker inside, bestP_beta sorted, subopt zuker optimized version
// string BeamCKYParser::get_parentheses_inside_real_backtrace(int i, int j, State& state, map<tuple<BestTypes, int, int>, string>& global_visited_inside, set<pair<int,int> >& window_visited) {
...
//         string left(state.trace.paddings.l1-1, '.');
//         string right(state.trace.paddings.l2-1,'.');
...
//         string left(state_beta.trace.paddings.l1-1, '.');
//         string right(state_beta.trace.paddings.l2-1,'.');
...
//         string right(state_beta.trace.split ,'.'); //this padding is not the same as others, we need to add '.' to replace the previous ')'.
...
int BeamCKYParser::parse(const string& seq, short**** internal_score, vector<int>* cons) {
    ...
    // start CKY decoding
#ifdef lv
        if(seq_length > 0) bestC[0].set(- v_score_external_unpaired(0, 0), LFMANNER_C_eq_C_plus_U);
        if(seq_length > 1) bestC[1].set(- v_score_external_unpaired(0, 1), LFMANNER_C_eq_C_plus_U);
#else
        if(seq_length > 0) bestC[0].set(score_external_unpaired(0, 0), LFMANNER_C_eq_C_plus_U);
        if(seq_length > 1) bestC[1].set(score_external_unpaired(0, 1), LFMANNER_C_eq_C_plus_U);
#endif
    ++nos_C;
    ...
    for(int j = 0; j < seq_length; ++j) {
        ...
        unordered_map<int, LFState>& beamstepH = bestH[j];
        unordered_map<int, LFState>& beamstepMulti = bestMulti[j];
        unordered_map<int, LFState>& beamstepP = bestP[j];
        unordered_map<int, LFState>& beamstepM2 = bestM2[j];
        unordered_map<int, LFState>& beamstepM = bestM[j];
        LFState& beamstepC = bestC[j];
        ...
        // beam of H
        {
...

            {
                ...
                if (jnext != -1) {
                    ...
                    update_if_better(bestH[jnext][j], newscore, LFMANNER_H);
                    ++ nos_H;
                }
            }

            {
                ...
                for (auto &item : beamstepH) {
                    ...
                    {
                        update_if_better(beamstepP[i], state.score, LFMANNER_HAIRPIN);
                        ++ nos_P;
                    }
                    ...
                    if (jnext != -1) {
                        ...
                        // so no need to check the score
                        update_if_better(bestH[jnext][i], newscore, LFMANNER_H);
                        ++nos_H;
                    }
                }
            }
            ...
            for(auto& item : beamstepMulti) {
...
#endif
                    update_if_better(beamstepP[i], newscore, LFMANNER_P_eq_MULTI);
                    ++ nos_P;
                }
                ...
                // 1. extend (i, j) to (i, jnext)
                {
                    ...
                    if (jnext != -1) {
                        ...
                        // this candidate must be the best one at [i, jnext]
                        // so no need to check the score
                        update_if_better(bestMulti[jnext][i], newscore, LFMANNER_MULTI_eq_MULTI_plus_U,
                                         new_l1,
                                         new_l2
                        );
                        ++nos_Multi;
        ...
        // beam of P
        {
            ...
            for(auto& item : beamstepP) {
...
#endif
                    update_if_better(beamstepM[i], newscore, LFMANNER_M_eq_P);
                    ++ nos_M;
                }
                //printf(" M = P at %d\n", j); fflush(stdout);
                ...
                if(!use_cube_pruning) {
                    ...
                    if ( k > 0 && !bestM[k].empty()) {
                        ...
                        for (auto &m : bestM[k]) {
                                ...
                                update_if_better(beamstepM2[newi], newscore, LFMANNER_M2_eq_M_plus_P, k);
                                ++nos_M2;
                            }
#else
                ...
                // 4. C = C + P
                {
                    int k = i - 1;
                    if (k >= 0) {
                      LFState& prefix_C = bestC[k];
                      if (prefix_C.manner != LFMANNER_NONE) {
                        int nuck = nuci_1;
                        int nuck1 = nuci;
                        value_type newscore;
#ifdef lv
                            ...
                            newscore = score_external_paired(k+1, j, nuck, nuck1, nucj, nucj1, seq_length) +
                                prefix_C.score + state.score;
#endif
                        update_if_better(beamstepC, newscore, LFMANNER_C_eq_C_plus_P, k);
                        ++ nos_C;
                      }
                    } else {
                        value_type newscore;
...
#endif
                        update_if_better(beamstepC, newscore, LFMANNER_C_eq_C_plus_P, -1);
                        ++ nos_C;
                    }
                }
                //printf(" C = C + P at %d\n", j); fflush(stdout);
            ...
            for(auto& item : beamstepP) {
                ...
                while ((filled < beam || current_score == prev_score) && !heap.empty()) {
                    ...
                    if (beamstepM2[newi].manner == LFMANNER_NONE) {
                        ++filled;
                        update_if_better(beamstepM2[newi], newscore, LFMANNER_M2_eq_M_plus_P, k);
                        ++nos_M2;
                    } else {
        ...
        // beam of M2
        {
            ...
            for(auto& item : beamstepM2) {
                ...
                // 2. M = M2
                {
                    update_if_better(beamstepM[i], state.score, LFMANNER_M_eq_M2);
                    ++ nos_M;
                }
...
#endif
                            update_if_better(bestMulti[q][p], newscore, LFMANNER_MULTI,
                                             static_cast<char>(i - p),
                                             q - j);
                            ++ nos_Multi;
                        }
...
#endif
                    update_if_better(bestM[j+1][i], newscore, LFMANNER_M_eq_M_plus_U);
                    ++ nos_M;
                }
            }
        }
...
#endif
                update_if_better(bestC[j+1], newscore, LFMANNER_C_eq_C_plus_U);
                ++ nos_C;
            }
        }
    ...
    LFState& viterbi = bestC[seq_length-1];
...
void BeamCKYParser::outside(vector<int> next_pair[]){
    ...
    bestC_beta[seq_length-1].manner = LFMANNER_NONE;
    // from right to left
    value_type newscore;
    for(int j = seq_length-1; j >= 0; --j) {
        int nucj = nucs[j];
        ...
        unordered_map<int, LFState>& beamstepH_beta = bestH_beta[j];
        unordered_map<int, LFState>& beamstepMulti_beta = bestMulti_beta[j];
        unordered_map<int, LFState>& beamstepP_beta = bestP_beta[j];
        unordered_map<int, LFState>& beamstepM2_beta = bestM2_beta[j];
        unordered_map<int, LFState>& beamstepM_beta = bestM_beta[j];
        LFState& beamstepC_beta = bestC_beta[j];
...
#ifdef lv
                update_if_better(beamstepC_beta, bestC_beta[j+1].score, LFMANNER_C_eq_C_plus_U);           
#else
                newscore = score_external_unpaired(j+1, j+1);
                update_if_better(beamstepC_beta, bestC_beta[j+1].score + newscore, LFMANNER_C_eq_C_plus_U);
#endif          
                // update
                // out_C[j+1] = beamstepC_beta.score;
            }
        }
        ...
        // beam of M
        {
            for(auto& item : beamstepM) {
                ...
                int i = item.first;
                LFState& state = beamstepM_beta[i];


                if (j < seq_length-1) {
#ifdef lv            
                    if (bestM_beta[j+1][i].manner != 0){
                        update_if_better(state, bestM_beta[j+1][i].score, LFMANNER_M_eq_M_plus_U);
                    }
#else
                    if (bestM_beta[j+1][i].manner != 0){
                        newscore = score_multi_unpaired(j + 1, j + 1);
                        update_if_better(state, bestM_beta[j+1][i].score + newscore, LFMANNER_M_eq_M_plus_U);
                    }
#endif
                    // update
                    // out_M[j+1][i+1] = state.score;
                    // cout << "outside score out_M[j+1][i+1]: " << i+1 << " " << j+1 << " " << state.score << endl;
                }
            }
        }
        ...
        // beam of M2
        {
            for(auto& item : beamstepM2) {
                int i = item.first;
                LFState& state = beamstepM2_beta[i];
                    ...
                    for (int p = i-1; p >= max(i - SINGLE_MAX_LEN, 0); --p) {
                        ...
                        if (q != -1 && ((i - p - 1) <= SINGLE_MAX_LEN)) {
                            ...
                            if (bestMulti_beta[q][p].manner != 0){
                                update_if_better(state, bestMulti_beta[q][p].score, LFMANNER_MULTI, static_cast<char>(i - p), q - j);
                            }
#else
     
                            if (bestMulti_beta[q][p].manner != 0){
                                newscore = score_multi_unpaired(p+1, i-1) + score_multi_unpaired(j+1, q-1);
                                update_if_better(state, bestMulti_beta[q][p].score + newscore, LFMANNER_MULTI, static_cast<char>(i - p), q - j);
                            }
#endif
                        }
                    }
                ...
                if (beamstepM_beta[i].manner != 0){
                    update_if_better(state, beamstepM_beta[i].score, LFMANNER_M_eq_M2);
                }

                // update
                // out_M2[j+1][i+1] = state.score;
                // cout << "outside score out_M2[j+1][i+1]: " << j+1 << " " << i+1 << " " <<  state.score << endl;
            }
        }
        // beam of P
        {  
            for(auto& item : beamstepP) {
                int i = item.first;
                LFState& state = beamstepP_beta[i];
                        ...
                        while (q != -1 && ((i - p) + (q - j) - 2 <= SINGLE_MAX_LEN)) {
                            ...
                            if (p == i - 1 && q == j + 1) {
                                ...
                                if (bestP_beta[q][p].manner != 0){
                                    ...
                                    update_if_better(state, (bestP_beta[q][p].score + score_single), LFMANNER_HELIX, static_cast<char>(i - p), q - j);
                                }
#else
                                if (bestP_beta[q][p].manner != 0){    
                                    newscore = score_helix(nucp, nucp1, nucq_1, nucq);
                                    update_if_better(state, bestP_beta[q][p].score + newscore, LFMANNER_HELIX, static_cast<char>(i - p), q - j);
                                }
                                ...
                                if  (bestP_beta[q][p].manner != 0){   
                                    int score_single = - v_score_single(p,q,i,j, nucp, nucp1, nucq_1, nucq, nuci_1, nuci, nucj, nucj1);
                                    update_if_better(state, (bestP_beta[q][p].score + score_single), LFMANNER_SINGLE, static_cast<char>(i - p), q - j);
                                }
#else
                                if (bestP_beta[q][p].manner != 0){    
                                    ...
                                    update_if_better(state, bestP_beta[q][p].score + newscore, LFMANNER_SINGLE, static_cast<char>(i - p), q - j);
                                }
                ...
                if(i > 0 && j < seq_length-1){
                        ...
                        if (beamstepM_beta[i].manner != 0){
                            ...
                            update_if_better(state, (beamstepM_beta[i].score + score_M1), LFMANNER_M_eq_P);
                        }
#else
                        if (beamstepM_beta[i].manner != 0){    
                            newscore = score_M1(i, j, j, nuci_1, nuci, nucj, nucj1, seq_length);
                            update_if_better(state, beamstepM_beta[i].score + newscore, LFMANNER_M_eq_P);
                        }
#endif
                }
                // 3. M2 = M + P
                ...
                if ( k > 0 && !bestM[k].empty()) {
                    ...
                    for (auto &m : bestM[k]) {
                        int newi = m.first;
                        LFState& m_state = bestM_beta[k][newi];

                        auto m_state_alpha = m.second.score;
                        if (beamstepM2_beta[newi].manner != 0){
                            update_if_better(m_state, (beamstepM2_beta[newi].score + m1_plus_P_alpha), LFMANNER_M2_eq_M_plus_P, j); //newi, k = i-1, i, j
                            update_if_better(state, (beamstepM2_beta[newi].score + m_state_alpha + m1_alpha), LFMANNER_M2_eq_M_plus_P, newi); // newi, k = i-1, i, j

                            // update
                            // out_M[k+1][newi+1] = m_state.score;
                            // cout << "outside score out_M[k+1][newi+1]: " << newi+1 << " " << k+1 <<" " << m_state.score << endl;
                        }
                    }
                }
                // 4. C = C + P
                {   
                    ...
                    if (k >= 0) {
                        ...
                        update_if_better(bestC_beta[k], state_alpha + external_paired_alpha_plus_beamstepC_beta, LFMANNER_C_eq_C_plus_P, j); //0, k, i, j
                        update_if_better(state, bestC[k].score + external_paired_alpha_plus_beamstepC_beta, LFMANNER_C_eq_C_plus_P, -1); // 0, k, i, j
                        // update
                        // out_C[k+1] = bestC_beta[k].score;
                        ...
                        int score_external_paired = - v_score_external_paired(0, j, -1, nucs[0],
                                                                 nucj, nucj1, seq_length);
                        update_if_better(state, (beamstepC_beta.score + score_external_paired), LFMANNER_C_eq_C_plus_P, -1); //C does not exist
#else
                        newscore = score_external_paired(0, j, -1, nucs[0],
                                                             nucj, nucj1, seq_length);
                        update_if_better(state, beamstepC_beta.score + newscore, LFMANNER_C_eq_C_plus_P, -1); //C does not exist
                ...
                // update
                // out_P[j+1][i+1] = state.score;
                // cout << "outside score out_P[j+1][i+1]: " << j+1 << " " << i+1 <<" " << state.score << endl;
            }
        }
        ...
        // beam of Multi
        {
            for(auto& item : beamstepMulti) {
                int i = item.first;
                LFState& state = beamstepMulti_beta[i];
                ...
                // 1. extend (i, j) to (i, jnext)
                {
                    if (jnext != -1) {
                        ...
                        if (bestMulti_beta[jnext][i].manner != 0){
                            update_if_better(state, (bestMulti_beta[jnext][i].score), LFMANNER_MULTI_eq_MULTI_plus_U, jnext - j);
                        }
#else
                        if (bestMulti_beta[jnext][i].manner != 0){
                            newscore = score_multi_unpaired(j, jnext - 1);
                            update_if_better(state, bestMulti_beta[jnext][i].score + newscore, LFMANNER_MULTI_eq_MULTI_plus_U, jnext - j);
                        }
#endif
                    }
                }
                ...
                // 2. generate P (i, j)
                {
                    ...
                    if (beamstepP_beta[i].manner != 0){
                        ...
                        update_if_better(state, (beamstepP_beta[i].score + score_multi), LFMANNER_P_eq_MULTI);
                    }
#else    
                    if (beamstepP_beta[i].manner != 0){
                        newscore = score_multi(i, j, nuci, nuci1, nucs[j-1], nucj, seq_length);
                        update_if_better(state, beamstepP_beta[i].score + newscore, LFMANNER_P_eq_MULTI);
                    }
                ...
                // update
                // if (j>180) cout << i+1 << " " << j+1 << " " << state.score << " " << state.manner << endl;
                // out_Multi[j+1][i+1] = state.score;
                // cout << "outside score out_Multi[k+1][newi+1]: " << j+1 << " " << i+1 <<" " << state.score << endl;
            }
        }
        ...
        // beam of H
        {
            for (auto &item : beamstepH) {
                int i = item.first;
                LFState &state = beamstepH_beta[i];
                int nuci = nucs[i];
                int jnext = next_pair[nuci][j];
                
                // 1. extend h(i, j) to h(i, jnext)
                if (jnext != -1 && jnext < seq_length && bestH_beta[jnext][i].score != VALUE_MIN) {
                    int nuci1 = (i + 1) < seq_length ? nucs[i + 1] : -1;
                    int nucjnext = nucs[jnext];
                    int nucjnext_1 = (jnext - 1) > -1 ? nucs[jnext - 1] : -1;
                    ...
                    // this candidate must be the best one at [i, jnext]
                    // so no need to check the score
                    update_if_better(state, bestH_beta[jnext][i].score + newscore, LFMANNER_H);
                    // cout << j << " " << i << " " << newscore << " " << bestH_beta[jnext][i].score << endl;
                }

                // 2. generate p(i, j)
                {
                    update_if_better(state, beamstepP_beta[i].score, LFMANNER_HAIRPIN);
                }

                // update
                // out_H[j+1][i+1] = state.score;
                // cout << "outside score out_H[k+1][newi+1]: " << j+1 << " " << i+1 << " " << state.score << endl;
            }
        }
