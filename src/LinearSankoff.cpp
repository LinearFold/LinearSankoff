...
tuple<string, string, string, string> BeamSankoffParser::get_parentheses_M2(SeqObject& seq1, SeqObject& seq2, stack<tuple<int, int, int, int, State>> &stk, BeamAlign &hmmalign) {
    ...
    if (!stk.empty()){
        ...
        switch (state.manner) {
            ...
            {
                cout << "MANNER_M2_eq_M_plus_P1" << endl;

                tuple<string, string, string, string> branch_insertion = backtrace_branch_insertion(k1+1, j1, k2, j2);
            ...
            {
                cout << "MANNER_M2_eq_M_plus_P2" << endl;

                tuple<string, string, string, string> branch_insertion = backtrace_branch_insertion(k1, j1, k2+1, j2);
...
string BeamSankoffParser::backtrace_single_seq(SeqObject* seq, int i, int j, vector<unordered_map<int, int> > &seq_in_P) {
    ...
    for (int p=i+1; p<=min(i+SINGLE_MAX_LEN, j-1); p++) {
        int nucp = seq->nucs[p];
        int nucp_1 = seq->nucs[p-1]; 

        int i_p = i - p;
        int q = seq->next_pair[nucp][max(p+3, j-SINGLE_MAX_LEN)];
        while (q != -1 && q < j && i_p + j - q + 2 > SINGLE_MAX_LEN) {
                ...
                while (j2next != -1) {
                    ...
                    if (alignscore > LOG_OF_ZERO)
                        update_if_better(j1, j1next, j2, j2next,
                                        bestH[j1next+j2next][j1next][make_pair(j1, j2)].alnobj, //bestH[j1next+j2next][get_keys(j1next, j1, j2)].alnobj,
                    ...
                    if (ins1score > LOG_OF_ZERO)
                        update_if_better(j1, j1next, j2, j2next,
                                        bestH[j1next+j2next][j1next][make_pair(j1, j2)].ins1obj, // bestH[j1next+j2next][get_keys(j1next, j1, j2)].ins1obj,
                    ...
                    if (ins2score > LOG_OF_ZERO)
                        update_if_better(j1, j1next, j2, j2next,
                                        bestH[j1next+j2next][j1next][make_pair(j1, j2)].ins2obj, // bestH[j1next+j2next][get_keys(j1next, j1, j2)].ins2obj,
...
// #endif
                        ...
                        // 1. extend (i, j) to (i, jnext)
                        {
                            ...
                            while (j1next != -1 && j1next <= min(seq1_len, j1+5)) {
                                ...
                                if (j1next == -1 || new_seq1_l2 > SINGLE_MAX_LEN) break;

                                // seq1 insertion 
                                if (m == 0 && j1next != -1 && j2 >= hmmalign.low_bounds[j1next] && j2 <= hmmalign.up_bounds[j1next] && (abs(new_seq1_l2 - state.trace2.paddings.l2) < 10)) {
                                    float alignscore = get_hmm_score_right(j1, j1next, j2, j2-1, 0, 0, true); // true
                                ...
                                while (j1next != -1 && j2next != -1 && j2next <= min(seq2_len, j2+5)) {
                                    ...
                                    if (j2next != -1 && j2next >= hmmalign.low_bounds[j1next] && j2next <= hmmalign.up_bounds[j1next] && (abs(new_seq1_l2 - new_seq2_l2) < 10)) // length of aligned multiloop are close 
                                    {
                                        switch (m) {
                                            case 2: 
                                            {
                                                alignscore = get_hmm_score_right(j1, j1next, j2, j2next, m, m, true); // true
                                                alignscore = xlog_mul(state.alignscore, alignscore);
                                                if (alignscore > LOG_OF_ZERO)
                                            ...
                                            case 0:
                                            {
                                                alignscore = get_hmm_score_right(j1, j1next, j2, j2next-1, m, m, true); // true
                                                alignscore = xlog_mul(state.alignscore, alignscore);
                                                if (alignscore > LOG_OF_ZERO)
                                            ...
                                            case 1:
                                            {
                                                alignscore = get_hmm_score_right(j1, j1next-1, j2, j2next, m, m, true); // true
                                                alignscore = xlog_mul(state.alignscore, alignscore);
                                                if (alignscore > LOG_OF_ZERO)
...
//                             if (new_seq1_l2 > SINGLE_MAX_LEN || new_seq2_l2 > SINGLE_MAX_LEN) break;
...
// #endif
                                        ...
                                        // // new feature: stem extension 
                                        // if (alnstate.manner != MANNER_STACKING_ALIGN_INTERNAL_LOOP2) {
                                        //     // case1: align with dots; ALN -> ALN 
                                        //     {
                                        //         int p2 = i2 - (i1 - p1);
                                        //         int q2 = j2 + (q1 - j1);
                                        //         // alignment check
                                        //         if (q2 >= hmmalign.low_bounds[q1] && q2 <= hmmalign.up_bounds[q1]) {
                                        //             // single seq folding check
                                        ...
                                        //     // case2: fully inserted; ALN -> INS1
                                        //     {
                                        //         int p2 = i2;
                                        //         int q2 = j2;
                                        //         // alignment check
                                        //         if (q2 >= hmmalign.low_bounds[q1] && q2 <= hmmalign.up_bounds[q1]) {
                                        //             pre_align_trans = get_hmm_score_left(p1, i1, p2, i2, 2, 0);
                                        //             post_align_trans = get_hmm_score_right(j1, q1, j2, q2, 0, 2);
                                                            
                                        //             alignscore = xlog_mul(pre_align_trans, alnstate.alignscore);
                                        //             alignscore = xlog_mul(alignscore, post_align_trans);

                                        //             if (alignscore > LOG_OF_ZERO)
                                        //                 update_if_better(p1, q1, p2, q2, bestP[q1+q2][q1][make_pair(p1, p2)].ins1obj,//bestP[q1+q2][get_keys(q1, p1, p2)].alnobj, 
                                        //                                 alnstate.seq1foldscore + p2p1,
                                        //                                 alnstate.seq2foldscore,
                                        //                                 alnstate.manner, MANNER_STACKING_ALIGN_INTERNAL_LOOP1, 
                                        //                                 static_cast<char>(i1 - p1), q1 - j1,
                            ...
                            // stem extension 
                            // switch (alnstate.manner) {
                            //     case MANNER_STACKING_ALIGN_INTERNAL_LOOP1:
                            //         {
                            //             int min_p1 = max(i1 - MAX_LOOP_LEN - 1, 1);
                            //             for (int p1 = i1-1; p1 >= min_p1; --p1) {
                            //                 int nucp1 = seq1->nucs[p1];
                            ...
                            //             break;
                            //         }
                            //     case MANNER_STACKING_ALIGN_INTERNAL_LOOP2:
                            //         break;
                            //     default:
                            //         break;
                            // }
                    ...
                    if (ins2state.manner != MANNER_NONE) {
                        ...
                        int max_p2 = min(i2-1, hmmalign.up_bounds[p1]);
                        int min_p2 = max(hmmalign.low_bounds[p1], max(1, i2 - 10)); // MAX_LOOP_LEN->10: lengths of aligned internal loops are close 
                    ...
                    if (ins1state.manner != MANNER_NONE) {
                        ...
                        int min_p1 = max(i1 - 10, 1); // MAX_LOOP_LEN => 10: lengths of aligned internal loops are close
                        for (int p1 = i1-1; p1 >= min_p1; --p1) {
                    ...
                    if (j2 >= hmmalign.low_bounds[i1] && j2 <= hmmalign.up_bounds[i1]) {
                        ...
                        if (k1 > 1 && k2 > 1 && k2 >= hmmalign.low_bounds[k1] && k2 <= hmmalign.up_bounds[k1] && !bestM[k1+k2][k1].empty()) {
                            ...
                            for (auto &m : bestM[k1+k2][k1]) {
                                ...
                                if (seq1_out_M2[j1].find(newi1) != seq1_out_M2[j1].end() && seq2_out_M2[j2].find(newi2) != seq2_out_M2[j2].end()) 
                                {
                                    // if (verbose) cout << "M2=M+P: " << i1 << " " << j1 << " "  << j2 << " " << j2 <<  " " << newi1 << " " << j1 << " "  << newi2 << " " << j2  << endl;
    // #ifdef dynalign
    //                             aln_value_type alignscore = alnstate.alignscore + mstate.alignscore;
    // #else
    ...
    // #endif 
                                    if (alignscore > LOG_OF_ZERO)
                                        update_if_better(newi1, j1, newi2, j2, beamM2[j1][make_pair(newi1, newi2)],
                                                        ...
                                                        alignscore, MANNER_ALN, MANNER_INS1, weight, verbose);     
                                }
                            }
                        } // M + P1
                                ...
                                if (prefix_C.endHMMstate != HMMMANNER_NONE) {
                                    ...
                                    if (alignscore > LOG_OF_ZERO)
                                        update_if_better(0, j1, 0, j2, beamC[j1].ins1obj,
                                                        ...
                                                        prefix_C.manner, MANNER_C_eq_C_plus_P1,
                                                        k1, k2,
                                                        alignscore, MANNER_INS2, MANNER_INS1, weight, verbose);
                                }
                            }
                        }  // C + P1
                    } // valid j2
                ...
                for(auto& item : seq2_in_P[j2]) {
                    ...
                    if (i2 >= hmmalign.low_bounds[j1] && i2 <= hmmalign.up_bounds[j1]) {
                        ...
                        if (k1 > 1 && k2 > 1 && k2 >= hmmalign.low_bounds[k1] && k2 <= hmmalign.up_bounds[k1] && !bestM[k1+k2][k1].empty()) {
                            int newscore2 = branch_score(i2, j2, seq2);
                            float ins1_aln_score = get_hmm_score_right(j1, j1, i2, j2, 2, 1); // fully inserted i2--j2
                            float alignscore;
                            ...
                            if (alignscore > LOG_OF_ZERO) {
                                for (auto &m : bestM[k1+k2][k1]) {
                                    ...
                                    if (seq1_out_M2[j1].find(newi1) != seq1_out_M2[j1].end() && seq2_out_M2[j2].find(newi2) != seq2_out_M2[j2].end()) 
                                    {
                                        if (verbose) cout << "M2=M+P: " << j1 << " " << j1 << " "  << i2 << " " << j2 <<  " " << newi1 << " " << j1 << " "  << newi2 << " " << j2  << endl;
    // #ifdef dynalign
    //                             aln_value_type alignscore = alnstate.alignscore + mstate.alignscore;
    // #else
                                        alignscore = xlog_mul(mstate.alignscore, ins1_aln_score); 
    // #endif 
                                        if (alignscore > LOG_OF_ZERO)
                                            update_if_better(newi1, j1, newi2, j2, beamM2[j1][make_pair(newi1, newi2)],
                                                            ...
                                                            alignscore, MANNER_ALN, MANNER_INS2, weight, verbose);     
                                ...
                                if (prefix_C.endHMMstate != HMMMANNER_NONE) {
                                    ...
                                    if (alignscore > LOG_OF_ZERO)
                                        update_if_better(0, j1, 0, j2, beamC[j1].ins2obj,
                                                        ...
                                                        prefix_C.manner, MANNER_C_eq_C_plus_P2,
                                                        k1, k2,
                                                        alignscore, MANNER_INS1, MANNER_INS2, weight, verbose);
                                }
                            }
                            {
                                State& prefix_C = bestC[k1+k2][k1].ins2obj;
        ...
        if (verbose) cout << "beam of M2" << endl;
        {
            ...
            for (int j1=hmmalign.min_j1[s]; j1<=hmmalign.max_j1[s]; j1++) {
                ...
                for (auto &item : beamM2[j1]) {
                    ...
                    HMMManner right_manner = state.endHMMstate;
                    int i1 = state.i1;
                    int i2 = state.i2;
                                ...
                                for (int p2 = max_p2; p2 >= min_p2; --p2) {
                                    ...
                                    while (q2 <= hmmalign.up_bounds[q1] && q2 != -1 && (q2 - j2 <= 5)) {
...
#else
                                            pre_align_trans = get_hmm_score_left(p1, i1, p2, i2, 2, 2);
                                            post_align_trans = get_hmm_score_right(j1, q1, j2, q2, right_manner-1, 2, true); // true
                                        ...
                                        // if (p2+1-hmmalign.low_bounds[p1] >= 0)
                                        {
                                            ...
                                            post_align_trans = get_hmm_score_right(j1, q1, j2, q2-1, right_manner-1, 0, true); // true
                                        ...
                                        // if (p2-hmmalign.low_bounds[p1+1] >= 0)
                                        {
                                            ...
                                            post_align_trans = get_hmm_score_right(j1, q1-1, j2, q2, right_manner-1, 1, true); // true
                    ...
                    if (seq1_out_M[j1].find(i1) != seq1_out_M[j1].end() && seq2_out_M[j2].find(i2) != seq2_out_M[j2].end()) 
                    {
                        ...
                        update_if_better(i1, j1, i2, j2, beamM[j1][make_pair(i1, i2)], // bestM[key1][newi2][newj2], //beamM[j2][make_pair(i1, i2)], 
                                        state.seq1foldscore,
                                        state.seq2foldscore,
