...
void BeamCKYPFParser::output_to_file(string file_name, const char * type) {
    if(!file_name.empty()) {
...
void BeamCKYPFParser::output_to_file_MEA_threshknot_bpseq(string file_name, const char * type, map<int,int>& pairs, const string & seq) {
...
void BeamCKYPFParser::cal_PairProb(PFState& viterbi) {
    ...
    for(int j=0; j<seq_length; j++){
        for(auto &item : bestP[j]){
            ...
            if (temp_prob_inside > pf_type(-9.91152)) {
                ...
                // cout << i << " " << j << " " << prob  << " " << temp_prob_inside<< endl;
            }
        }
    }
...
string BeamCKYPFParser::back_trace(const int i, const int j, const vector<vector<int> >& back_pointer){
...
map<int, int> BeamCKYPFParser::get_pairs(string & structure){
    map<int, int> pairs;
    stack<int> s;
    int index = 1;
    int pre_index = 0;
...
void BeamCKYPFParser::ThreshKnot(const string & seq){
...
void BeamCKYPFParser::PairProb_MEA(const string & seq) {
...
void BeamCKYPFParser::outside(vector<int> next_pair[]){
