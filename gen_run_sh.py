...
if "four" in fam or "five" in fam:
    ...
    out_dir = "/scratch/lisiz/lso_ret/SRP_case_study/w%s_b%s_LFb%s_energyDiff%s_ret" % (w, b, LFb, energyDiff)
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

elif "time_mem_data" == fam:
    data_dir = "/nfs/stak/users/lisiz/LinearSankoff/dataset/"
    out_dir = "/scratch/lisiz/lso_ret/time_mem_data/w%s_b%s_LFb%s_energyDiff%s_ret" % (w, b, LFb, energyDiff)
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

else:
    data_dir = "/nfs/stak/users/lisiz/LinearSankoff/dataset/train_data"
    out_dir = "/scratch/lisiz/lso_ret/multi_right_jump/train_w%s_b%s_LFb%s_energyDiff%s_ret" % (w, b, LFb, energyDiff)
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    if fam in ["16S", "SRP", "RNaseP", "telomerase"]:
        data_dir = "/nfs/stak/users/lisiz/LinearSankoff/dataset/test_data"
        out_dir = "/scratch/lisiz/lso_ret/multi_right_jump/test_w%s_b%s_LFb%s_energyDiff%s_ret" % (w, b, LFb, energyDiff)
